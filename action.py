...
KOBO_FIRMWARE_UPDATE_CHECK_URL = (
    "https://api.kobobooks.com/1.0/UpgradeCheck/Device/{0}/{1}/{2}/{3}"
)
...
class KoboUtilitiesAction(InterfaceAction):
    ...
    def library_changed(self, db):
        ...
        if self.haveKobo() and self.current_device_profile:
            if self.current_device_profile[cfg.STORE_OPTIONS_STORE_NAME][
    ...
    def _on_device_metadata_available(self):
        ...
        if self.haveKobo():
            debug_print(
                "KoboUtilites:_on_device_metadata_available - self.current_device_profile:",
                self.current_device_profile,
            )
            debug_print(
                "KoboUtilites:_on_device_metadata_available - self.current_backup_config:",
                self.current_backup_config,
            )
            if (
                self.current_backup_config[cfg.KEY_DO_DAILY_BACKUP]
            ...
            if (
                self.current_device_profile
                and self.current_device_profile[cfg.STORE_OPTIONS_STORE_NAME][
                    cfg.KEY_STORE_ON_CONNECT
                ]
            ):
                debug_print(
            ...
            if (
                self.haveKoboTouch()
                and self.current_firmware_check_config[cfg.KEY_DO_UPDATE_CHECK]
            ):
                debug_print(
                    "KoboUtilities:_on_device_metadata_available - About to do firmware check"
                )
    ...
    def rebuild_menus(self):
        with self.menus_lock:
            ...
            self.menu.addSeparator()
            self.firmware_update_action = self.create_menu_item_ex(
                self.menu,
                _("Check for Kobo Updates") + "...",  # shortcut=False,
    ...
    def should_check_kobo_updates(self):
        last_check = self.current_firmware_check_config[
            cfg.KEY_LAST_FIRMWARE_CHECK_TIME
        ]
    ...
    def menu_firmware_update_check(self):
        ...
        if not self.should_check_kobo_updates() and not question_dialog(
            self.gui,
            _("Kobo Firmware Update"),
            _(
                "You last checked for a Kobo update less than a day ago. Do you want to check again now?"
            ),
            show_copy_button=False,
        ):
            return
    ...
    def set_time_on_device(self):
        ...
        epoch_conf_path = os.path.join(
            self.device_path, KOBO_ROOT_DIR_NAME, KOBO_EPOCH_CONF_NAME
        )
    ...
    def auto_firmware_update_check(self, from_menu=False):
        ...
        if self.should_check_kobo_updates():
            options = cfg.get_plugin_prefs(
                cfg.UPDATE_OPTIONS_STORE_NAME, fill_defaults=True
            )
            options[cfg.KEY_LAST_FIRMWARE_CHECK_TIME] = calendar.timegm(time.gmtime())
            cfg.plugin_prefs[cfg.UPDATE_OPTIONS_STORE_NAME] = options
            self.device_path = self.get_device_path()

            kobo_update_file = os.path.join(
                self.device_path, KOBO_ROOT_DIR_NAME, "KoboRoot.tgz"
            )
            kobo_manifest_sums_file = os.path.join(
                self.device_path, KOBO_ROOT_DIR_NAME, "manifest.md5sum"
            )
            kobo_upgrade_dir = os.path.join(
                self.device_path, KOBO_ROOT_DIR_NAME, "upgrade"
            )
            if os.path.exists(kobo_update_file):
                if not question_dialog(
                    ...
                    _("Kobo Firmware Update"),
                    _(
                        "The KoboRoot.tgz file is already in place for an upgrade. Do you want to check for updates anyway?"
                    ),
                    show_copy_button=False,
                ):
                    return
            ...
            try:
                ...
                is_beta_user = kobo_config.has_section(
                    "FeatureSettings"
                ) and kobo_config.getboolean("FeatureSettings", "AcceptPreviewUpgrades")
            except ValueError:
                is_beta_user = False
            except NoOptionError:
                is_beta_user = False
            if is_beta_user:
                beta_continue_anyway = question_dialog(
                    self.gui,
                    _("Kobo Firmware Update"),
                    _(
                        "You appear to be a beta tester. Upgrade checking currently only works for official firmware releases. Do you wish to continue checking for official release firmware updates?"
                    ),
                    show_copy_button=False,
                )
                if not beta_continue_anyway:
                    debug_print(
                        "do_check_firmware_update - beta tester is not checking for firmware"
                    )
                    return
            ...
            if version_info:
                ...
                affiliate = "whsmith"
                affiliate_file = os.path.join(
                    self.device_path, KOBO_ROOT_DIR_NAME, "affiliate.conf"
                )
                if not self.current_firmware_check_config[
                    ...
                    if affiliate_config.has_section(
                        "General"
                    ) and affiliate_config.has_option("General", "affiliate"):
                        affiliate = affiliate_config.get("General", "affiliate")
                serial_no = self.device_serial_no()
                update_url = KOBO_FIRMWARE_UPDATE_CHECK_URL.format(
                    version_info[5], affiliate, version_info[2], serial_no
                )
                ...
                if resp.getcode() == 200:
                    ...
                    if update_data["UpgradeURL"] is not None:
                        m = re.search(
                            r"\/kobo\-update\-(?P<version>\d+\.\d+\.\d+)\.zip$",
                            update_data["UpgradeURL"],
                        )
                        if m:
                            upgrade_version = m.group("version")
                        else:
                            upgrade_version = "(Unknown)"
                        upgrade_continue = question_dialog(
                            self.gui,
                            _("Kobo Firmware Update"),
                            _(
                                "A Kobo firmware update to version {0} is available. Do you want to update? You have version {1}.".format(
                                    upgrade_version, version_info[2]
                                )
                            ),
                            show_copy_button=False,
                        )
                        ...
                        debug_print(
                            "auto_firmware_update_check - No firmware upgrade available"
                        )
                        if from_menu:
                            info_dialog(
                                self.gui,
                                _("Kobo Firmware Update"),
                                _(
                                    "Kobo firmware update check complete - no updates available"
                                ),
                                show=True,
                                show_copy_button=False,
                            )
                    ...
                    raise ValueError(
                        "Couldn't check for firmware update: got HTTP%s"
                        % resp.getcode()
                    )
        else:
            debug_print(
                "auto_firmware_update_check - Not checking for firmware, only checking once per day"
            )
    ...
    def auto_backup_device_database(self, from_menu=False):
        ...
        backup_options[cfg.KEY_BACKUP_COPIES_TO_KEEP] = self.current_backup_config[
            cfg.KEY_BACKUP_COPIES_TO_KEEP
        ]
        backup_options[cfg.KEY_DO_DAILY_BACKUP] = self.current_backup_config[
            cfg.KEY_DO_DAILY_BACKUP
        ]
        backup_options[cfg.KEY_BACKUP_EACH_CONNECTION] = self.current_backup_config[
            cfg.KEY_BACKUP_EACH_CONNECTION
        ]
        backup_options[cfg.KEY_BACKUP_ZIP_DATABASE] = self.current_backup_config[
    ...
    def remove_annotations_files(self):
        ...
        debug_print(
            "remove_annotations_files - self.device_path='%s'" % (self.device_path)
        )

        self.options["annotations_dir"] = self.device.normalize_path(
            self.device_path + "Digital Editions/Annotations/"
        )
    ...
    def mark_not_interested(self):
        ...
        for book in books:
            ...
            if "Recommendation" in book.device_collections:
                ...
                debug_print(
                    "mark_not_interested - book.device_collections:",
        ...
        updated_books, unchanged_books, not_on_device_books, count_books = (
            self._update_metadata(recommendations)
        )
        result_message = _(
            "Books marked as Not Interested:\n\tBooks updated={0}\n\tUnchanged books={1}\n\tTotal books={2}"
        ).format(updated_books, unchanged_books, count_books)
        info_dialog(
            self.gui,
            _("Kobo Utilities") + " - " + _("Device library updated"),
            result_message,
    ...
    def clean_images_dir(self):
        ...
        if self.device.fwversion >= self.device.min_fwversion_images_tree:
            self.main_image_path = os.path.join(
                self.device._main_prefix, ".kobo-images"
            )
            self.sd_image_path = (
                os.path.join(self.device._card_a_prefix, "koboExtStorage/images-cache/")
            ...
            self.sd_image_path = (
                os.path.join(self.device._card_a_prefix, "koboExtStorage/images")
                if self.device._card_a_prefix
                else None
            )
            self.options["images_tree"] = False
        self.options["main_image_path"] = self.device.normalize_path(
        ...
        if ContentType == 6:
            ...
            if extension == ".kobo":
                ...
                ContentID = ContentID.replace(
                    self.device._main_prefix
                    + self.device.normalize_path(".kobo/kepub/"),
                    "",
                )
            else:
                ...
                ContentID = path
                ContentID = ContentID.replace(
                    self.device._main_prefix, "file:///mnt/onboard/"
                )
            ...
            if self.device._card_a_prefix is not None:
                ContentID = ContentID.replace(
                    self.device._card_a_prefix, "file:///mnt/sd/"
                )
        else:  # ContentType = 16
    ...
    def get_device(self):
        ...
        if self.device:
            ...
            debug_print(
                'KoboUtilities:get_device - self.device_path="%s"' % self.device_path
            )
            get_current_device_information = (
                self.gui.device_manager.get_current_device_information()
            )
            if (
                self.device_path == "" or not get_current_device_information
            ):  # No device actually connected or it isn't ready
                self.device = None
                return self.device
            self.connected_device_info = get_current_device_information.get(
                "info", None
            )
            ...
            self.current_device_profile = cfg.get_book_profile_for_device(
                library_db, self.device_uuid, use_any_device=True
            )
            self.current_device_config = cfg.get_device_config(self.device_uuid)
            device_name = cfg.get_device_name(self.device_uuid, None)
            self.device_name = (
                device_name if device_name is not None else self.device.gui_name
            )
            debug_print("KoboUtilities:get_device - device_name:", self.device_name)
            self.individual_device_options = cfg.get_plugin_pref(
                cfg.COMMON_OPTIONS_STORE_NAME, cfg.KEY_INDIVIDUAL_DEVICE_OPTIONS
            )
            if self.individual_device_options:
                self.current_backup_config = cfg.get_prefs(
                    self.current_device_config, cfg.BACKUP_OPTIONS_STORE_NAME
                )
                self.current_firmware_check_config = cfg.get_prefs(
                    self.current_device_config, cfg.UPDATE_OPTIONS_STORE_NAME
                )
            else:
                self.current_backup_config = cfg.get_plugin_prefs(
                    cfg.BACKUP_OPTIONS_STORE_NAME, fill_defaults=True
                )
                self.current_firmware_check_config = cfg.get_plugin_prefs(
                    cfg.UPDATE_OPTIONS_STORE_NAME, fill_defaults=True
                )

        self.supports_series = (
            self.haveKoboTouch()
            and "supports_series" in dir(self.device)
            and self.device.supports_series()
        )
        self.supports_series_list = self.haveKoboTouch() and (
            "supports_series_list" in dir(self.device)
            and self.device.supports_series_list
        ...
        try:
            self.epub_location_like_kepub = (
                self.haveKoboTouch()
                and self.device_fwversion >= self.device.min_fwversion_epub_location
            )
        except:
            self.epub_location_like_kepub = (
                self.haveKoboTouch() and self.device_fwversion >= (4, 17, 13651)
            )
    ...
    def _firmware_update(self, update_data):
        ...
        args = [
            update_data,
            os.path.join(self.get_device_path(), KOBO_ROOT_DIR_NAME),
            cpus,
        ]
        desc = _("Downloading Kobo firmware update")
        job = self.gui.device_manager.create_job(
            do_check_firmware_update,
            self.Dispatcher(self._firmware_completed),
            description=desc,
            args=args,
        )
        job._tdir = None
    ...
    def _firmware_completed(self, job):
        if job.failed:
            self.gui.job_exception(
                job, dialog_title=_("Failed to update Kobo firmware")
            )
            return
        ...
        if update_results is True:
            debug_print(
                "KoboUtilitiesAction::_firmware_completed - Done updating Kobo firmware"
            )
            ret = question_dialog(
                self.gui,
                _("Kobo Firmware Update"),
                _("Keep Kobo firmware update files?"),
                show_copy_button=False,
            )
            if not ret:
                ...
                try:
                    ...
                    debug_print(
                        "WARNING: _firmware_completed - Couldn't remove 'upgrade' directory: %s"
                        % (str(e),)
                    )
                try:
                    ...
                    debug_print(
                        "WARNING: _firmware_completed - Couldn't remove 'manifest.md5sum' file: %s"
                        % (str(e),)
                    )
            else:
                info_dialog(
                    self.gui,
                    _("Kobo Firmware Update"),
                    _("Eject and unplug your Kobo device to complete the update"),
                    show=True,
                    show_copy_button=False,
                )
            ...
            error_dialog(
                self.gui,
                _("Kobo Firmware Update"),
                _("Exception encountered updating Kobo firmware"),
                det_msg=update_results.message,
                show=True,
            )
    ...
    def _remove_covers(self, books):
        with closing(self.device_database_connection()) as connection:
            ...
            debug_print(
                "_remove_covers - remove_fullsize_covers=", remove_fullsize_covers
            )
            ...
            for book in books:
                ...
                for contentID in contentIDs:
                    ...
                    try:
                        ...
                        debug_print(
                            "_remove_covers - no match for contentId='%s'"
                            % (contentID,)
                        )
    ...
    def _open_cover_image_directory(self, books):
        with closing(
            ...
            for book in books:
                ...
                debug_print(
                    "_open_cover_image_directory - book.__class__=", book.__class__
                )
                debug_print(
                    "_open_cover_image_directory - book.contentID=", book.contentID
                )
            ...
            for book in books:
                ...
                for contentID in contentIDs:
                    ...
                    try:
                        result = next(cursor)
                        debug_print(
                            "_test_covers - contentId='%s', imageId='%s'"
                            % (contentID, result["ImageId"])
                        )
                        ...
                        cover_dir = os.path.join(
                            path, ".kobo-images", "%s" % dir1, "%s" % dir2
                        )
                        ...
                        debug_print(
                            "_test_covers - no match for contentId='%s'" % (contentID,)
                        )
                        not_on_device_books += 1
                    total_books += 1
    ...
    def _check_book_in_database(self, books):
        with closing(self.device_database_connection()) as connection:
            ...
            for book in books:
                if not book.contentID:
                    book.contentID = self.contentid_from_path(
    ...
    def _order_series_shelves(self, shelves, options):
        ...
        with closing(
            ...
            for shelf in shelves:
                ...
                self.set_progressbar_label(
                    _("Updating shelf: {0}").format(shelf["name"])
                )
                self.increment_progressbar()
                if shelf["count"] <= 1:
                    continue
    ...
    def _set_related_books(self, related_books, options):
        ...
        with closing(
            ...
            for related_type in related_books:
                ...
                debug_print(
                    "_set_related_books - related_type_data:", related_type_data
                )
                ...
                for tab_contentId in related_type_contentIds:
                    ...
                    for volume_contentId in related_type_contentIds:
                        if not tab_contentId == volume_contentId:
                            ...
                            debug_print(
                                "_set_related_books - insert_data:", insert_data
                            )
    ...
    def _remove_duplicate_shelves(self, shelves, options):
        ...
        with closing(self.device_database_connection()) as connection:
            ...
            for shelf in shelves:
                ...
                finished_shelves += 1
                self.set_progressbar_label(
                    _("Removing duplicates of shelf ") + shelf[0]
                )
                self.increment_progressbar()
    ...
    def generate_metadata_query(self):
        debug_print(
            "generate_metadata_query - self.supports_series=%s, self.supports_series_list%s"
            % (self.supports_series, self.supports_series_list)
        )
            ...
            for book in books:
                ...
                for contentID in book.contentIDs:
                    ...
                    if result is not None:
                        debug_print(
                            "_update_metadata: found contentId='%s'" % (contentID)
                        )
                        ...
                        if (
                            ...
                            debug_print(
                                "_update_metadata: newmi.authors=", newmi.authors
                            )
                            debug_print(
                                "_update_metadata: newmi.comments=", newmi.comments
                            )
                        else:
                        ...
                        if (
                            ...
                            debug_print(
                                "_update_metadata: set_clause=", set_clause_columns
                            )
                        ...
                        if self.options[cfg.KEY_SET_NOT_INTERESTED] and not (
                            result["FeedbackType"] == 2
                            or result["FeedbackTypeSynced"] == 1
                        ):
                        ...
                        if self.supports_series and self.options["series"]:
                            ...
                            debug_print(
                                '_update_metadata: new_series="%s"' % (new_series,)
                            )
                            debug_print(
                            ...
                            debug_print(
                                '_update_metadata: series_changed="%s"'
                                % (series_changed,)
                            )
                            ...
                            debug_print(
                                "_update_metadata: self.supports_series_list='%s'"
                                % self.supports_series_list
                            )
                            if self.supports_series_list:
                                ...
                                debug_print(
                                    "_update_metadata: series_id='%s'" % series_id
                                )
                                if (
                                    series_changed
    ...
    def _store_current_bookmark(self, books, options=None):
        ...
        with closing(
            ...
            debug_print(
                "_store_current_bookmark - rating_column_name=", rating_column_name
            )
            debug_print(
            ...
            for book in books:
                ...
                mi = Metadata("Unknown")
                debug_print(
                    "_store_current_bookmark - Looking at book: %s" % book.title
                )
                ...
                for contentID in book.contentIDs:
                    debug_print(
                        "_store_current_bookmark - contentId='%s'" % (contentID)
                    )
                    ...
                    if result is not None:
                        ...
                        if result["ReadStatus"] == 0:
                            ...
                            if (
                                last_read_column_name is not None
                                and store_if_more_recent
                            ):
                                ...
                                if (
                                    ...
                                    current_last_read is not None and last_read is None
                                ):
                                    continue
            ...
            for book in books:
                ...
                for contentID in book.contentIDs:
                    ...
                    if result is not None:
                        ...
                        debug_print(
                            "_restore_current_bookmark - last_read= ", last_read
                        )
                        debug_print(
                        ...
                        debug_print(
                            "_restore_current_bookmark - chapter_update=%s"
                            % chapter_update
                        )
                        debug_print(
                            "_restore_current_bookmark - chapter_values= ",
        ...
        with closing(self.device_database_connection()) as connection:
            ...
            for book in books:
                ...
                if update_library and len(shelf_names) > 0:
                    ...
                    debug_print(
                        "_get_shelves_from_device - library shelf names='%s'"
                        % (old_value)
                    )
                    if old_value is None or not set(old_value) == set(shelf_names):
                        debug_print(
                            "_get_shelves_from_device - shelves are not the same"
                        )
                        shelf_names = (
                        ...
                        if replace_shelves or old_value is None:
                            ...
                            new_value = old_value + ", " + shelf_names
                        debug_print(
                            "_get_shelves_from_device - new shelf names='%s'"
                            % (new_value)
                        )
                        library_db.set_custom(
                            book.calibre_id,
    ...
    def fetch_book_fonts(self):
        ...
        with closing(self.device_database_connection()) as connection:
            ...
            if result is not None:
                ...
                book_options["readingAlignment"] = (
                    result[2].title() if result[2] else "Off"
                )
    ...
    def _set_reader_fonts(self, contentIDs, delete=False):
        ...
        with closing(self.device_database_connection()) as connection:
            ...
            if not delete:
                ...
                justification = (
                    None
                    if justification == "Off" or justification == ""
                    else justification
                )
    ...
    def _backup_annotation_files(self, books, dest_path):
        ...
        debug_print(
            "_backup_annotation_files - self.device_path='%s'" % (self.device_path)
        )
        kepub_dir = self.device.normalize_path(".kobo/kepub/")
        annotations_dir = self.device.normalize_path(
            self.device_path + "Digital Editions/Annotations/"
        )
    ...
    def _get_chapter_status(self, db, books):
        ...
        with closing(
            ...
            for book in books:
                ...
                ContentType = (
                    self.device.get_content_type_from_extension(extension)
                    if extension != ""
                    else self.device.get_content_type_from_path(device_book_path)
                )
                book["ContentID"] = self.device.contentid_from_path(
                ...
                if db.has_format(book_id, book["kobo_format"], index_is_id=True):
                    ...
                    book["kobo_format"] == "KEPUB"
                    and "EPUB".lower() in self.device.settings().format_map
                ...
                if len(book["kobo_database_chapters"]) == 0:
                    debug_print(
                        "_get_chapter_status - No chapters in database for book."
                    )
                    book["can_update_toc"] = False
                    book["kobo_database_status"] = False
    ...
    def _compare_toc_entries_old(self, book, book_format="EPUB"):
        for i, library_chapter in enumerate(book["library_chapters"]):
            ...
            if not (
                library_chapter_path == book["kobo_database_chapters"][i]["path"]
                and kobo_chapter_path == book["kobo_database_chapters"][i]["path"]
            ):
                debug_print(
                    "_compare_toc_entries - path different for chapter index: %d" % i
                )
                debug_print(
                    "_compare_toc_entries - library_chapter='%s'"
                    % library_chapter["path"]
                )
                debug_print(
                    "_compare_toc_entries - kobo_database_chapters='%s'"
                    % book["kobo_database_chapters"][i]["path"]
                )
                debug_print(
                    "_compare_toc_entries - kobo_chapters='%s'"
                    % book["kobo_chapters"][i]["path"]
                )
                return False
            if not (
                library_chapter["title"] == book["kobo_database_chapters"][i]["title"]
                and book["kobo_chapters"][i]["title"]
                == book["kobo_database_chapters"][i]["title"]
            ):
                debug_print(
                    "_compare_toc_entries - title different for chapter index: %d" % i
                )
    ...
    def update_device_toc_for_books(self, books):
        ...
        with closing(self.device_database_connection()) as connection:
            for book in books:
                ...
                debug_print(
                    "update_device_toc_for_books - ContentID=", book["ContentID"]
                )
                self.increment_progressbar()
