...
from calibre.ebooks.oeb.base import OEB_DOCS, SVG, XLINK, XPath, urlnormalize
...
class ParseError(ValueError):
    def __init__(self, name, desc):
        ...
        ValueError.__init__(
            self,
            _("Failed to parse: %(name)s with error: %(err)s")
            % dict(name=name, err=desc),
        )
...
class Container(object):
    ...
    def __init__(self, path, log):
        ...
        for item in self.opf.xpath(
            "//opf:manifest/opf:item[@href and @media-type]", namespaces={"opf": OPF_NS}
        ):
            href = item.get("href")
    ...
    def manifest_worthy_names(self):
        for name in self.name_path_map:
            ...
            if (
                ...
                and posixpath.basename(name) in self.META_INF
            ):
                continue
            yield name
    ...
    def get_manifest_item_for_name(self, name):
        ...
        existing = self.opf.xpath(
            '//opf:manifest/opf:item[@href="%s"]' % q, namespaces={"opf": OPF_NS}
        )
        if not existing:
            return None
        return existing[0]
    ...
    def get_guide_reference(self, ref_type):
        ...
        """
        references = self.opf.xpath(
            '//opf:guide/opf:reference[@type="%s"]' % ref_type,
    ...
    def get_manifest_item_by_id(self, id):
        ...
        """
        items = self.opf.xpath(
            '//opf:manifest/opf:item[@id="%s"]' % id, namespaces={"opf": OPF_NS}
        )
        if len(items) > 0:
            return items[0]
        return None
    ...
    def get_meta_content_item(self, name):
        meta_items = self.opf.xpath(
            '//opf:metadata/opf:meta[@name="%s" and @content]' % name,
            namespaces={"opf": OPF_NS},
        )
        if len(meta_items):
            return meta_items[0]
    ...
    def decode(self, data):
        ...
        if data[:4] in ("\0\0\xfe\xff", "\xff\xfe\0\0"):
            bom_enc = {"\0\0\xfe\xff": "utf-32-be", "\xff\xfe\0\0": "utf-32-le"}[
                data[:4]
            ]
            data = data[4:]
...
class WritableContainer(Container):
    ...
    def generate_manifest_id(self):
        items = self.opf.xpath(
            "//opf:manifest/opf:item[@id]", namespaces={"opf": OPF_NS}
        )
    ...
    def generate_unique(self, id=None, href=None):
        ...
        if href is not None:
            items = self.opf.xpath(
                "//opf:manifest/opf:item[@href]", namespaces={"opf": OPF_NS}
            )
    ...
    def add_to_manifest(self, id, href, mt=None):
        ...
        manifest = self.opf.xpath("//opf:manifest", namespaces={"opf": OPF_NS})[0]
        item = manifest.makeelement(
            "{%s}item" % OPF_NS, nsmap={"opf": OPF_NS}, href=href, id=id
        )
        if not mt:
    ...
    def add_to_spine(self, id, index=-1):
        ...
        itemref = spine.makeelement(
            "{%s}itemref" % OPF_NS, nsmap={"opf": OPF_NS}, idref=id
        )
        if index >= 0:
    ...
    def add_to_guide(self, href, title, ref_type):
        ...
        reference = etree.SubElement(
            guide, "{%s}reference" % OPF_NS, attrib=attrib, nsmap={"opf": OPF_NS}
        )
    ...
    def delete_from_manifest(self, name, delete_from_toc=True):
        ...
        manifest = self.opf.xpath("//opf:manifest", namespaces={"opf": OPF_NS})[0]
        self.log(
            "\t  Manifest item removed: %s (%s)" % (item.get("href"), item.get("id"))
        )
    ...
    def delete_from_spine(self, item):
        ...
        """
        item_id = item.get("id")
        itemrefs = self.opf.xpath(
            '//opf:spine/opf:itemref[@idref="%s"]' % item_id, namespaces={"opf": OPF_NS}
        )
        if len(itemrefs) > 0:
    ...
    def delete_from_guide(self, item):
        ...
        references = self.opf.xpath(
            '//opf:guide/opf:reference[@href="%s"]' % item_href,
            namespaces={"opf": OPF_NS},
        )
        if len(references):
    ...
    def set(self, name, val):
        if hasattr(val, "xpath"):
            ...
            val = etree.tostring(val, encoding="utf-8", xml_declaration=True)
        else:
    ...
    def write(self, path):
        ...
        with ZipFile(path, "w", compression=ZIP_DEFLATED) as zf:
            # Write mimetype
            zf.writestr(
                "mimetype", bytes(guess_type("a.epub")[0]), compression=ZIP_STORED
            )
            ...
            for root, _dirs, files in os.walk(self.root):
                for fn in files:
                    ...
                    zfn = os.path.relpath(absfn, self.root).replace(os.sep, "/")
                    zf.write(absfn, zfn)
...
class ExtendedContainer(WritableContainer):
    ...
    def flatten_toc(self):
        ...
        nested = self.ncx.xpath(
            r"descendant::ncx:navPoint/ncx:navPoint", namespaces={"ncx": NCX_NS}
        )
        if len(nested) == 0:
