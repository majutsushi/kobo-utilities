...
TILE_TYPES = {  # Format is: Activity/Tile name, Display Name, tooltip
    ...
    (
        ...
        _("Lists several categories from the Kobo Bookstore."),
    ),
    ("Extras", _("Extras"), _("A tile is displayed for each extra when used.")),
    (
        "GlobalStats",
        _("Global Stats"),
        _("Displays the number of finished books in your library."),
    ),
    ("Library", _("Library"), _("Shows new books added to the library.")),
    (
        "QuickTour",
        _("Quick Tour"),
        _("The device Quick Tour that is displayed when the device is first set-up."),
    ),
    ...
    (
        ...
        _("Related Items"),
        _(
            "After a sync, will show books related to any you are reading. There can be one tile for each of your books."
        ),
    ),
    (
        "WhatsNew",
        _("Release Notes"),
        _(
            "Shows that there was an update to the firmware with the new version number. You probably don't want to dismiss this."
        ),
    ),
    ...
    (
        "Sync",
        _("Sync"),
        _("Displays when a sync was last done. Does not have options to dismiss it."),
    ),
...
EXTRAS_TILES = {  # Format is: Activity/Tile name, Display Name, tooltip
    (
        "chess",
        _("Chess"),
        _("Take on your eReader in the classic game of strategy and skill."),
    ),
    (
        "sketch",
        _("Sketch Pad"),
    ...
    (
        "browser",
        _("Web Browser"),
        _("Take the on-ramp to the information superhighway, right on your eReader."),
    ),
    (
        "rushhour",
        _("Unblock It"),
        _("Liberate your block by moving other blocks out of the way."),
    ),
    (
        "solitaire",
        _("Solitaire"),
        _("Play classic solitaire games like Klondike, Spider, and Freecell."),
    ),
...
class UpdateMetadataOptionsDialog(SizePersistedDialog):
    ...
    def ok_clicked(self):
        ...
        self.new_prefs[cfg.KEY_USE_TITLE_SORT] = (
            self.title_sort_checkbox.checkState() == Qt.Checked
        )
        self.new_prefs[cfg.KEY_SET_AUTHOR] = (
            self.author_checkbox.checkState() == Qt.Checked
        )
        self.new_prefs[cfg.KEY_USE_AUTHOR_SORT] = (
            self.author_sort_checkbox.checkState() == Qt.Checked
        )
        self.new_prefs[cfg.KEY_SET_DESCRIPTION] = (
            self.description_checkbox.checkState() == Qt.Checked
        )
        self.new_prefs[cfg.KEY_DESCRIPTION_USE_TEMPLATE] = (
            self.description_use_template_checkbox.checkState() == Qt.Checked
        )
        self.new_prefs[cfg.KEY_DESCRIPTION_TEMPLATE] = (
            self.description_template_edit.template
        )
        self.new_prefs[cfg.KEY_SET_PUBLISHER] = (
            self.publisher_checkbox.checkState() == Qt.Checked
        )
        self.new_prefs[cfg.KEY_SET_PUBLISHED_DATE] = (
            self.published_checkbox.checkState() == Qt.Checked
        )
        self.new_prefs[cfg.KEY_SET_ISBN] = self.isbn_checkbox.checkState() == Qt.Checked
        self.new_prefs[cfg.KEY_SET_RATING] = (
            self.rating_checkbox.checkState() == Qt.Checked
        )
        self.new_prefs[cfg.KEY_SET_SERIES] = (
            self.series_checkbox.checkState() == Qt.Checked
        )
        self.new_prefs[cfg.KEY_USE_PLUGBOARD] = (
            self.use_plugboard_checkbox.checkState() == Qt.Checked
        )
        self.new_prefs[cfg.KEY_SET_LANGUAGE] = (
            self.language_checkbox.checkState() == Qt.Checked
        )
        self.new_prefs[cfg.KEY_UDPATE_KOBO_EPUBS] = (
            self.update_kepubs_checkbox.checkState() == Qt.Checked
        )
        self.new_prefs[cfg.KEY_SET_SUBTITLE] = (
        ...
        self.new_prefs[cfg.KEY_SET_READING_DIRECTION] = (
            self.reading_direction_checkbox.checkState() == Qt.Checked
        )
        self.new_prefs[cfg.KEY_SYNC_DATE] = (
        ...
        if self.readingStatusGroupBox.readingStatusIsChecked():
            ...
            self.new_prefs[cfg.KEY_RESET_POSITION] = (
        ...
        for key in self.new_prefs:
            debug_print(
                "UpdateMetadataOptionsDialog:ok_clicked - key='%s' self.new_prefs[key]=%s"
                % (key, self.new_prefs[key])
            )
            if (
                self.new_prefs[key]
...
class GetShelvesFromDeviceDialog(SizePersistedDialog):
    ...
    def ok_clicked(self):
        ...
        self.options[cfg.KEY_ALL_BOOKS] = (
            self.all_books_checkbox.checkState() == Qt.Checked
        )
        self.options[cfg.KEY_REPLACE_SHELVES] = (
...
class DismissTilesOptionsDialog(SizePersistedDialog):
    def __init__(self, parent, plugin_action):
        SizePersistedDialog.__init__(
            self, parent, "kobo utilities plugin:dismiss tiles settings dialog"
        )
        self.plugin_action = plugin_action
        ...
        self.tiles_new_checkbox.setCheckState(
            Qt.Checked
            if self.options.get(cfg.KEY_TILE_RECENT_NEW, False)
            else Qt.Unchecked
        )
        self.tiles_finished_checkbox.setCheckState(
            Qt.Checked
            if self.options.get(cfg.KEY_TILE_RECENT_FINISHED, False)
            else Qt.Unchecked
        )
        self.tiles_inthecloud_checkbox.setCheckState(
            Qt.Checked
            if self.options.get(cfg.KEY_TILE_RECENT_IN_THE_CLOUD, False)
            else Qt.Unchecked
        )
    ...
    def initialize_controls(self):
        ...
        self._add_groupbox(
            ...
            "Tile Types:",
            TILE_TYPES,
            self.options.get(cfg.KEY_TILE_OPTIONS, {}),
        )
        col2_layout.addSpacing(5)

        options_group = QGroupBox(_("Book Tiles"), self)
        options_group.setToolTip(
            _("For books, you can dismiss the 'Finished' and 'New' tiles.")
        )
        layout.addWidget(options_group)
        options_layout = QGridLayout()
        options_group.setLayout(options_layout)
        ...
        self.tiles_new_checkbox.setToolTip(
            _(
                "Select this option if you want to dismiss new books. This will act on all tiles of this type."
            )
        )
        options_layout.addWidget(self.tiles_new_checkbox, 0, 0, 1, 1)
        self.tiles_finished_checkbox = QCheckBox(_("Finished"), self)
        self.tiles_finished_checkbox.setToolTip(
            _("Select this option if you want to dismiss finished books.")
        )
        ...
        self.tiles_inthecloud_checkbox.setToolTip(
            _(
                "Select this option if you want to dismiss books that are 'In the Cloud'."
            )
        )
        ...
        options_group.setToolTip(
            _(
                "When a tile is added or changed, the database trigger will automatically set them to be dismissed. This will be done for the tile types selected above."
            )
        )
        layout.addWidget(options_group)
        options_layout = QGridLayout()
        ...
        self.database_trigger_checkbox.setToolTip(
            _("Select this option if you want to change the current database trigger.")
        )
        options_layout.addWidget(self.database_trigger_checkbox, 0, 0, 1, 2)
        self.database_trigger_checkbox.clicked.connect(
            self.database_trigger_checkbox_clicked
        )
        ...
        button_box.rejected.connect(self.reject)
        self.select_none_button = button_box.addButton(
    ...
    def _add_groupbox(self, layout, title, option_info, options):
        ...
        groupbox.setToolTip(
            _(
                "This is the list of Tile types that can be dismissed. Select the one you want to dismiss."
            )
        )
        ...
        for key, text, tooltip in sorted(option_info):
            ...
            checkbox.setCheckState(
                Qt.Checked if options.get(key, False) else Qt.Unchecked
            )
    ...
    def _ok_clicked(self):
        ...
        for option_name, _t, _tt in TILE_TYPES:
            self.options[cfg.KEY_TILE_OPTIONS][option_name] = (
                getattr(self, option_name).checkState() == Qt.Checked
            )

        self.options[cfg.KEY_TILE_RECENT_NEW] = (
            self.tiles_new_checkbox.checkState() == Qt.Checked
        )
        self.options[cfg.KEY_TILE_RECENT_FINISHED] = (
            self.tiles_finished_checkbox.checkState() == Qt.Checked
        )
        self.options[cfg.KEY_TILE_RECENT_IN_THE_CLOUD] = (
            self.tiles_inthecloud_checkbox.checkState() == Qt.Checked
        )

        cfg.plugin_prefs[cfg.DISMISSTILES_OPTIONS_STORE_NAME] = self.options

        self.options[cfg.KEY_CHANGE_DISMISS_TRIGGER] = (
            self.database_trigger_checkbox.checkState() == Qt.Checked
        )
        self.options[cfg.KEY_CREATE_DISMISS_TRIGGER] = (
        ...
        if (
            ...
            or self.options[cfg.KEY_TILE_RECENT_NEW]
            or self.options[cfg.KEY_DELETE_DISMISS_TRIGGER]
...
class DispayExtrasTilesDialog(SizePersistedDialog):
    def __init__(self, parent, plugin_action):
        SizePersistedDialog.__init__(
            self, parent, "kobo utilities plugin:display extras tiles dialog"
        )
        self.plugin_action = plugin_action
        ...
        self.dismiss_current_extras_checkbox.setCheckState(
            Qt.Checked
            if self.options.get(cfg.KEY_DISMISS_CURRENT_EXTRAS, False)
            else Qt.Unchecked
        )
    ...
    def initialize_controls(self):
        ...
        self.dismiss_current_extras_checkbox = QCheckBox(
            _("Dismiss current Extras tiles"), self
        )
        self.dismiss_current_extras_checkbox.setToolTip(
            _(
                "Select this option if you want to dismiss the Extras tiles already on the home screen."
            )
        )
    ...
    def _ok_clicked(self):
        ...
        for option_name, _t, _tt in EXTRAS_TILES:
            ...
            have_options = (
                have_options or self.options[cfg.KEY_TILE_OPTIONS][option_name]
            )

        self.options[cfg.KEY_DISMISS_CURRENT_EXTRAS] = (
            self.dismiss_current_extras_checkbox.checkState() == Qt.Checked
        )
...
class BlockAnalyticsOptionsDialog(SizePersistedDialog):
    ...
    def ok_clicked(self):
        self.options = {}
        self.options[cfg.KEY_CREATE_ANALYTICSEVENTS_TRIGGER] = (
            self.create_trigger_radiobutton.isChecked()
        )
        self.options[cfg.KEY_DELETE_ANALYTICSEVENTS_TRIGGER] = (
            self.delete_trigger_radiobutton.isChecked()
        )
...
class ShowReadingPositionChangesDialog(SizePersistedDialog):
    ...
    def _ok_clicked(self):
        ...
        self.prefs[cfg.KEY_SELECT_BOOKS_IN_LIBRARY] = (
            self.select_books_checkbox.checkState() == Qt.Checked
        )
        self.prefs[cfg.KEY_UPDATE_GOODREADS_PROGRESS] = (
...
class OrderSeriesShelvesDialog(SizePersistedDialog):
    def __init__(self, parent, plugin_action, shelves):
        ...
        self.order_shelves_by = self.options.get(
    ...
    def initialize_controls(self):
        ...
        self.order_shelves_type_button_group.buttonClicked.connect(
            self._order_shelves_type_radio_clicked
        )
        layout.addSpacing(5)
        ...
        self.order_shelves_by_button_group.buttonClicked.connect(
            self._order_shelves_by_radio_clicked
        )
        ...
        self.order_shelves_in_button_group.buttonClicked.connect(
    ...
    def _ok_clicked(self):
        ...
        self.options[cfg.KEY_SORT_DESCENDING] = (
            self.order_shelves_in
        )  # self.descending_radiobutton.isChecked()
        self.options[cfg.KEY_SORT_UPDATE_CONFIG] = (
            self.update_config_checkbox.isChecked()
        )
    ...
    def fetch_button_clicked(self):
        self.shelves = self.plugin_action._get_series_shelf_count(
            self.order_shelves_type
        )
        self.shelves_table.populate_table(self.shelves)
        return
